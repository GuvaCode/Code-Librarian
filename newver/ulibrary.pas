...
uses
   ...
   SynEdit, SynHighlighterCpp, SynHighlighterHTML, SynHighlighterSQL,LCLType;
...
uses
 SysUtils, Messages;//, GX_CodeSrch, GX_CodeOpt;
...
procedure TCodeFrm.SetLayout(New: TCodeLayout);
begin
  if FLayout <> New then
  begin
    FLayout := New;
...
begin
 ...
 // actEditPaste.Enabled := (Clipboard.HasFormat(CF_TEXT) and (not FCodeText.ReadOnly));
  ...
  SnippetIsSelected := HaveSelectedNode and
      (tvTopics.Selected.ImageIndex = CodeSnippetImageIndex);

  actEditPasteToIde.Enabled := SnippetIsSelected;
  actEditCopyFromIde.Enabled := SnippetIsSelected;
...
begin
  ...
  FCodeText.ReadOnly := True;
  FCodeText.Gutter.Visible:=false;
...
begin
  ...
  try
   ...
   // DatabasePath := AddTrailingPathDelimiter(DatabasePath);
   // {$IFOPT D+}SendDebug('Database path: ' + DatabasePath);{$ENDIF}
    FLayout := clTop;//clSide;
     SetLayout(FLayout);
    FModified := False;

 CodeDB := OpenDB(DefaultDBFileName); // do not localize
 if CodeDB = nil then CodeDB := createNewDb(DefaultDBFileName);

   // LoadSettings;

 {   CodeDB := OpenDB(DatabasePath + DefaultDBFileName); // do not localize
    if CodeDB = nil then
      CodeDB := CreateNewDB(DatabasePath + DefaultDBFileName); // do not localize
       }
    if CodeDB = nil then
    begin
      MessageDlg(SCouldNotCreateDatabase, mtError, [mbOK], 0);
