...
Const
     SYNS_FilterProlog               =  'Prolog Files (*.pro,*.pl,*.P)|*.pro;*.pl;*.P';
...
function TSynPrologSyn.GetDefaultAttribute(Index: integer
...
procedure TSynPrologSyn.GetTokenEx(out TokenStart: PChar; out
  TokenLength: integer);
begin
...
function TSynPrologSyn.GetSampleSource: string;
begin
  Result := 'member(X,[X| _]).'+#13#10+
            'member(X,[_ |T]) :- member(X,T).'+#13#10+
            ''+#13#10+
            'length([], 0).'+#13#10+
            'length([A|B],N) :- length(B,M), N is M+1 .'+#13#10+
            ''+#13#10+
            'smallest([A],A) :- !.'+#13#10+
            'smallest([A|B],N) :- smallest(B,N), N<A, !.'+#13#10+
            'smallest([A| _ ], A).'+#13#10+
            ''+#13#10+
            'depth(X,0) :- atomic(X), !.'+#13#10+
            'depth([X|Y],N) :- depth(X,XD), depth(Y,YD), X1 is XD+1, max(X1,YD,N).'+#13#10+
            'max(X,Y,X) :- X>Y, !.'+#13#10+
            'max(_,Y,Y).';
end;
